...
<style>
  ...
  .stCaption {{ color: #c0cad6 !important; }}
  ...
  [data-testid="stRadio"] [data-baseweb="radio"] > div:first-child {{ border-color: #c8a96e !important; }}
  [data-testid="stRadio"] [data-baseweb="radio"][aria-checked="true"] > div:first-child {{ background-color: #c8a96e !important; border-color: #c8a96e !important; }}
  [role="radio"][aria-checked="true"] > div:first-child {{ background: #c8a96e !important; border-color: #c8a96e !important; }}
  [data-testid="stCheckbox"] label {{ color: #f0f4f8 !important; }}
  [data-testid="stButton"] button[kind="primary"] {{ background: linear-gradient(135deg, #c8a96e 0%, #a07840 100%) !important; color: #0d1a33 !important; font-weight: 700 !important; border: none !important; font-size: 16px !important; }}
  [data-testid="stButton"] button[kind="primary"]:hover {{ background: linear-gradient(135deg, #d4b87e 0%, #b08850 100%) !important; }}
...
<div style="background:linear-gradient(135deg,#0d1a33 0%,#1a2744 100%);border-bottom:3px solid #c8a96e;padding:20px 36px;margin:-5rem -5rem 2rem -5rem;display:flex;align-items:center;justify-content:space-between;">
  <div style="display:flex;align-items:center;gap:28px;">
    <img src="data:image/png;base64,{LOGO_B64}" style="height:34px;width:auto;filter:brightness(0) invert(1);" />
    <div style="width:1px;height:40px;background:rgba(200,169,110,0.4);"></div>
    <div>
      <div style="font-size:14px;color:#f0f4f8;font-weight:600;letter-spacing:0.5px;">Armington Trade Model</div>
      <div style="font-size:11px;color:#c8a96e;letter-spacing:1.5px;text-transform:uppercase;margin-top:3px;">MIT Capstone 2026</div>
    </div>
  </div>
  <div style="font-size:11px;color:#a0b0c0;letter-spacing:1px;text-transform:uppercase;text-align:right;line-height:1.8;">Investments that keep<br>the world moving</div>
</div>
...
def render_baseline(df,label,sigma,total):
    ...
    for c in ["#","Country","Baseline Share","Old Tariff"]: h+=f'<th style="padding:6px 8px">{c}</th>'
    h+='</tr>'
    for _,r in df.iterrows():
        ...
        h+=(f'<tr style="background:{bg};text-align:right">'
            ...
            f'<td style="padding:5px 8px;color:#f0f4f8">{_pct(r["OldTariff"])}</td></tr>')
