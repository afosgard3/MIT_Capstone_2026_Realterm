...
st.markdown("""
<style>
  /* ── Google Fonts — Michroma (closest free match to Bank Gothic) ── */
  ...
  [data-testid="stHeader"]  { background-color: #0f1117; border-bottom: 1px solid #1e2235; }
  ...
  .stMarkdown h1 { color: #ffffff !important; font-size: 2rem !important; }
  .stMarkdown h2 { color: #ffffff !important; }
  .stMarkdown h3 { color: #c8a96e !important; }
  .stCaption     { color: #c0cad6 !important; font-size: 12px !important; }

  /* ── Radio buttons — gold accent, no red ── */
  ...
  /* ── Number input labels ── */
  [data-testid="stNumberInput"] label { color: #c0cad6 !important; font-size: 12px !important; }
  ...
  [data-testid="stButton"] button[kind="primary"]:hover {
    background: linear-gradient(135deg, #d4b87e 0%, #b08850 100%) !important;
  ...
  /* ── Tables — dark background, bright text ── */
...
<div style="
  background: linear-gradient(135deg, #0d1a33 0%, #1a2744 100%);
...
">
  <div style="display:flex; align-items:center; gap:24px;">
    ...
    "></div>
    <div>
      <div style="font-size:14px; color:#f0f4f8; font-weight:600; letter-spacing:0.5px;">
        Armington Trade Model
      </div>
      <div style="font-size:11px; color:#c8a96e; letter-spacing:1.5px; text-transform:uppercase; margin-top:2px;">
        MIT Capstone 2026
      </div>
    </div>
  </div>
  <div style="
    ...
    line-height: 1.6;
  ">
    Investments that keep<br>the world moving
  </div>
</div>
...
if st.session_state.results is not None:
    ...
    def _build_results_csv(result_df):
        df = result_df.copy()
        df["BaselineShare_pct"]  = df["BaselineShare"].map(lambda v: f"{v*100:.2f}%")
        df["NewShare_pct"]       = df["NewShare"].map(lambda v: f"{v*100:.2f}%")
        df["ShareChange_pp"]     = df["ShareChangePct"].map(lambda v: f"{v*100:.3f}pp")
        df["OldTariff_pct"]      = df["OldTariff"].map(lambda v: f"{v*100:.2f}%")
        df["NewTariff_pct"]      = df["NewTariff"].map(lambda v: f"{v*100:.2f}%")
        df["TariffChange_pct"]   = df["TariffChange"].map(lambda v: f"{v*100:.2f}%")
        out = df[["Rank","Country of Origin",
                  "BaselineShare_pct","OldTariff_pct",
                  "NewTariff_pct","TariffChange_pct",
                  "NewShare_pct","ShareChange_pp",
                  "BaselineQty","NewQty","GrowthValue"]].copy()
        out.columns = ["Rank","Country","Baseline Share","Old Tariff",
                       "New Tariff","Tariff Change","New Share","Share Change (pp)",
                       "Baseline Qty ($)","New Qty ($)","Growth ($)"]
        return out.to_csv(index=False).encode("utf-8")

    def _build_port_csv(port_df, reg_df, total):
        port_df2 = port_df.copy()
        port_df2["Region"] = port_df2["Port of Entry"].apply(assign_region)
        port_df2["Before_pct"]   = port_df2["OldShare"].map(lambda v: f"{v*100:.3f}%")
        port_df2["After_pct"]    = port_df2["NewShare"].map(lambda v: f"{v*100:.3f}%")
        port_df2["Change_pp"]    = port_df2["Change"].map(lambda v: f"{v*100:.3f}pp")
        port_df2["Before_$"]     = port_df2["OldShare"] * total
        port_df2["After_$"]      = port_df2["NewShare"] * total
        port_df2["Volume_Delta"] = port_df2["Change"]   * total
        out = port_df2[["Port of Entry","Region","Before_pct","After_pct",
                        "Change_pp","Before_$","After_$","Volume_Delta"]]
        out.columns = ["Port","Region","Before (%)","After (%)","Change (pp)",
                       "Before ($)","After ($)","Volume Delta ($)"]
        return out.to_csv(index=False).encode("utf-8")
    ...
    with col_dl1:
        st.download_button(
            label="⬇️ Download Country Results (CSV)",
            data=_build_results_csv(result),
            file_name=f"armington_results_{label[:30].replace(' ','_')}.csv",
            mime="text/csv",
    ...
    if port_df is not None and not port_df.empty:
        with col_dl2:
            st.download_button(
                label="⬇️ Download Port Impact (CSV)",
                data=_build_port_csv(port_df, reg_df, total),
                file_name=f"port_impact_{label[:30].replace(' ','_')}.csv",
                mime="text/csv",
                use_container_width=True,
            )

    with col_dl3:
        # Combined workbook — both sheets as separate CSVs zipped
        import zipfile, io as _io
        zip_buf = _io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w") as zf:
            zf.writestr("country_results.csv", _build_results_csv(result).decode("utf-8"))
            if port_df is not None and not port_df.empty:
                zf.writestr("port_impact.csv", _build_port_csv(port_df, reg_df, total).decode("utf-8"))
        st.download_button(
            label="⬇️ Download All (ZIP)",
            data=zip_buf.getvalue(),
            file_name=f"armington_full_{label[:30].replace(' ','_')}.zip",
