...
st.markdown(f"""
<div style="background:linear-gradient(135deg,#0d1a33 0%,#1a2744 100%);border:1px solid #2d3a5c;border-bottom:3px solid #c8a96e;border-radius:10px;padding:28px 48px;margin-bottom:32px;display:flex;align-items:center;justify-content:space-between;">
  <div style="display:flex;align-items:center;gap:32px;">
    <img src="data:image/png;base64,{LOGO_B64}" style="height:56px;width:auto;filter:brightness(0) invert(1);" />
    <div style="width:1px;height:56px;background:rgba(200,169,110,0.5);"></div>
    <div>
      <div style="font-size:20px;color:#f0f4f8;font-weight:700;letter-spacing:0.5px;">Armington Trade Model</div>
      <div style="font-size:12px;color:#c8a96e;letter-spacing:2.5px;text-transform:uppercase;margin-top:5px;">MIT Capstone 2026</div>
    </div>
  </div>
  <div style="font-size:12px;color:#8090a8;letter-spacing:1.5px;text-transform:uppercase;text-align:right;line-height:2.2;">Investments that keep<br>the world moving</div>
</div>
""", unsafe_allow_html=True)
...
def render_port_table(port_df,reg_df,total):
    def _row(name,old,new,chg,is_reg=False):
        cc=_cc(chg); s=_sign(chg)
        bg="#1e2740" if is_reg else ("#1a1d2e" if hash(name)%2 else "#13161f")
        fw="bold" if is_reg else "normal"
        # Color the change and volume delta cells with green/red badges like results table
        if chg > 0.0005:
            chg_bg,chg_fg = "#1a3d2a","#6fcf97"
            vol_bg,vol_fg = "#1a3d2a","#6fcf97"
        elif chg < -0.0005:
            chg_bg,chg_fg = "#3d1a1a","#f08080"
            vol_bg,vol_fg = "#3d1a1a","#f08080"
        else:
            chg_bg,chg_fg = bg,"#a0b0c0"
            vol_bg,vol_fg = bg,"#a0b0c0"
        return (f'<tr style="background:{bg};font-weight:{fw};text-align:right">'
                f'<td style="text-align:left;padding:5px 10px;color:#f0f4f8">{name}</td>'
                f'<td style="padding:5px 10px;color:#f0f4f8">{old*100:.2f}%</td>'
                f'<td style="padding:5px 10px;color:#f0f4f8">{new*100:.2f}%</td>'
                f'<td style="padding:5px 10px;background:{chg_bg};color:{chg_fg};font-weight:bold">{s}{chg*100:.2f}pp</td>'
                f'<td style="padding:5px 10px;color:#f0f4f8">{_dollar(old*total)}</td>'
                f'<td style="padding:5px 10px;color:#f0f4f8">{_dollar(new*total)}</td>'
                f'<td style="padding:5px 10px;background:{vol_bg};color:{vol_fg};font-weight:bold">{s}{_dollar(chg*total)}</td></tr>')
